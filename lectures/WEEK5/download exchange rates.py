...
def fx_code(from_cur, to_cur):
    ...
    cur_AAA = from_cur.upper().strip()
    cur_BBB = to_cur.upper().strip()

    if len(cur_AAA) != 3:
        raise Exception(f"from_curr must be ISO code, not {from_cur}")
    if len(cur_BBB) != 3:
        raise Exception(f"to_curr must be ISO code, not {to_cur}")

    if cur_AAA == 'USD':
        tic = f'{cur_BBB}=X'
    else:
        tic = f'{cur_AAA}{cur_BBB}=X'
    return tic
